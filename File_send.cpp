    ...
    : threadpool_(subthreadnum, "WORE"), sockfd_(sockfd), path_(path)
{
    ...
    qDebug() << "传入的filename是： " << qstr;
    strcpy(fileInformation_.filename, filename.c_str());

    qDebug() << "结构体中的文件名称是： " << fileInformation_.filename;
...
bool File_send::SendFile() // 发送文件
{
    ...
    while (true)
    {
        ...
        if (totalbytes == static_cast<int>(fileInformation_.filesize))
            break;
...
FileInformation File_send::fileInformation() //返回文件结构体
{
