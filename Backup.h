...
private:
    ...
    bool stopInit_ = false;

    

public:
    explicit Backup(QObject *parent = nullptr, const std::string &ip = "",
                    const std::string &port = "", const std::string &path = "", int worktheadnum = 0);
    ~Backup();
    ...
    void stopWork();                         // 停止工作
signals:
    void resultReady(FileInformation fileInformation, QString time);
